...
contract PairTest is Test {
    ...
    function testInitialize() public view {
        ...
        assertEq(asset1, address(token2));
    }
    ...
    function testAddLiquidity() public {
        ...
        token2.approve(address(pair), 300 ether);
        ...
        assertEq(token1.balanceOf(address(pair)), 500 ether);
        assertEq(token2.balanceOf(address(pair)), 300 ether);
    }

    function testAddLiquidityOneSided() public {
        vm.startPrank(lp);
        token1.approve(address(pair), 200 ether);
    ...
    function testSwapToken2ForToken1() public {
        ...
        token2.transfer(address(pair), 200 ether); // Input-Token an Pair senden
    ...
    function testSwapRevertsWhenNotEnoughLiquidity() public {
        vm.startPrank(user);
        token1.approve(address(pair), 500 ether);
        token2.approve(address(pair), 500 ether);
