...
contract InvariantRouter {
    ...
    function _swap(
        ...
        for (uint i = 0; i < path.length - 1; i++) {
            ...
            (address token0, ) = InvariantLibrary(lib).sortTokens(
                input,
                output
            );
            ...
            require(
                pair != address(0),
                "InvariantRouter: PAIR_NOT_FOUND_ERROR"
            );
            ...
            if (i < path.length - 2) {
                recipient = pair;
                console.log("Intermediate recipient (next pair): ", recipient);
            } else {
                recipient = to;
    ...
    function swapExactTokensForTokens(
        ...
        uint256[] memory amounts = InvariantLibrary(lib).getAmountsOut(
            factory,
            amountIn,
            path
        );
        ...
        require(
            ...
            "InvariantRouter: INSUFFICIENT_OUTPUT_AMOUNT"
        );
        address firstPair = InvariantFactory(factory).getPair(path[0], path[1]);
    ...
    function swapTokensForExactTokens(
        ...
        uint256[] memory amounts = InvariantLibrary(lib).getAmountsIn(
            factory,
            amountOut,
            path
        );
        require(
            ...
            "InvariantRouter: EXCESSIVE_INPUT_AMOUNT"
        );
        address firstPair = InvariantFactory(factory).getPair(path[0], path[1]);
        IERC20(path[0]).transferFrom(msg.sender, firstPair, amounts[0]);
        _swap(amounts, path, to);
